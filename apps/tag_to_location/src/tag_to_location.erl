...
%% -------------------------------------------------------------------
%
% Remove a tag and set as location in all matching documents.
%

-module(tag_to_location).
...
main([]) ->
    io:format("Usage: tag_to_location <config-file> <tag>~n"),
    ok.
...
% For the given document, remove the tag (in binary) and set as location.
process_one(Db, DocId, Tag) ->
    {ok, Doc} = couchbeam:open_doc(Db, DocId),
    % If the document has a location already, do not make any changes.
    case couchbeam_doc:get_value(<<"location">>, Doc) of
        L when L == undefined orelse L == null ->
            ...
            NewDoc2 = couchbeam_doc:set_value(<<"location">>, Tag, NewDoc1),
